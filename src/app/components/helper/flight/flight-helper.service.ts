...
export class FlightHelperService implements AfterContentInit, OnInit {
  ...
  ngOnInit(): void {
    ...
    if(localUserId?.detail.id){
     this.getLocationByCode(localUserId?.detail.id)
    }
  }

  public getLocationByCode(value:any,stops:any=null,bagges:any=null,airlineName:any=null) {
    this.locationsService.get_alll(value,stops,bagges,airlineName).subscribe(
      (res: any) => {
        this.flightblock = res;
